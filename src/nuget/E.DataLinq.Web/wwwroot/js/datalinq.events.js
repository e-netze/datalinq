dataLinq.eventController = function (baseObj) {
...
new dataLinq.implementEventController(dataLinq);

dataLinq.events.on('onselected', function (channel, sender, args) {
    var $view = $(args.element).closest('.datalinq-include, .datalinq-include-click');
    var $selected = $view.find(".datalinq-selected");
    if ($selected.length > 0)
        $view.find(".datalinq-mass-attribution").removeClass("datalinq-mass-attribution-deactivated");
    else
        $view.find(".datalinq-mass-attribution").addClass("datalinq-mass-attribution-deactivated");
});