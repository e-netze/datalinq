(function ($) {
    "use strict";
    $.fn.dataLinq_modal = function (method) {
        if (methods[method]) {
            ...
            $.error('Method ' + method + ' does not exist on jQuery.dataLinq_modal');
        }
    };
    var defaults = {
    ...
    var methods = {
        ...
        content: function (options) {
            var settings = $.extend({}, defaults, options);
            return $(dialogSelector(settings)).find('#datalinq-modal-content');
        },
        title: function (options) {
            var settings = $.extend({}, defaults, options);
            return $(dialogSelector(settings)).find('.datalinq-modal-title');
        },
        ...
        toggle_fullscreen: function (options) {
            options = $.extend({}, defaults, options);
            var elem = $(dialogSelector(options)).find('.datalinq-modal-body').get(0);
        ...
        fit: function (options) {
            ...
            if (useMobile)
                return;
            var $modalBody = $(dialogSelector(options)).find('.datalinq-modal-body');
            var $content = $modalBody.find('#datalinq-modal-content');
    ...
    var initUI = function (parent, options) {
        ...
        var $blocker = $("<div id='" + dialogId(options) + "' style='z-index:9999;position:" + blockerPosition + ";left:0px;right:0px;top:0px;bottom:0px;background:rgba(0,0,0," + options.blocker_alpha + ");' class='datalinq-modal'></div>");

        var $frame = $("<div id='" + (options.hasBlocker === true ? '' : dialogId(options)) + "' style='z-index:1000;position:absolute;" + framePos + "background:white;opacity:0;" + (useMobile === true ? "" : "display:none;") + "' class='datalinq-modal-body " + options.dock + "'></div>").appendTo($blocker);
        var pPos, mPos;
        if (useMobile === true) {
        ...
        var $content = $("<div id='datalinq-modal-content' class='datalinq-modal-content' style='z-index:1;position:absolute;overflow:auto;" + pPos + "'></div>");
        if (options.content)
        ...
        if (options.blockerclickclose) {
            ...
            $blocker.click(function (e) {
                ...
                if (Math.abs(dx) < 5 && Math.abs(dy) < 5) {
                    if ($(this).find('.datalinq-modal-close').length > 0)
                        $(this).find('.datalinq-modal-close').trigger('click');
                    else if ($(this).find('.datalinq-modal-close-element').length > 0)
                        $(this).find('.datalinq-modal-close-element').trigger('click');
                }
            });
        }
        ...
        var $title = $("<div style='position:absolute;left:0px;right:0px;top:0px;height:35px'><div class='datalinq-modal-title'>" + (options.title ? options.title : '') + "</div></div>"),
            $close = null;
        $title.appendTo($frame);
        if (options.closebutton) {
            $title.children('.datalinq-modal-title').addClass('has-closebutton');
            if (!isEnlargeable) {
                $("<div class='datalinq-modal-resize'></div>").appendTo($title)
            }
            $close = $("<div class='datalinq-modal-close'></div>");
            $close.appendTo($title).get(0).options = options;
            $close.click(function (e) {
                e.stopPropagation();
                if (options.onclose)
                    options.onclose();
                $(null).dataLinq_modal('close', this.options);
            });
        }
        if (isEnlargeable) {
            $frame.addClass('enlargeable');
            $title.click(function () {
                var $modal = $(this).closest('.datalinq-modal-body');
                var $blocker = $(this).closest('.datalinq-modal');
        ...
        if (useMobile === false && (options.onmaximize || options.allowfullscreen === true)) {
            ...
            if (options.onmaximize) {
                ...
                $max.click(function () { $(null).dataLinq_modal('toggle_fullscreen'); });
            }
        }
        ...
        dataLinq.delayed(function ($frame) {
            if (options.animate) {
    ...
    var dialogId = function (options) {
        if (options.id)
            return 'datalinq-modal-' + options.id.replace(/\./g, '-').replace(/:/g, '-');
        ;
    };
    var dialogSelector = function (options) {
        var id = dialogId(options);
        return (id === '' ? '.datalinq-modal' : '#' + id + '.datalinq-modal');
    };
})(jQuery);
